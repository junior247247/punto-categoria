Public Class frm_listas_desde
    ...
    Public Sub mostrar()
        ...
        Try
            ...
            lblOtrosGastos.Text = ParseToDecimal.parse(dt.Rows(0).Item("total").ToString)
            ...
            lblTotalInversion.Text = ParseToDecimal.parse(dt.Rows(0).Item("total").ToString)
            ...
            lbl_mantenimiento.Text = ParseToDecimal.parse(dt.Rows(0).Item("MANTENIMINETO").ToString)
            ...
            lbl_recargas.Text = ParseToDecimal.parse(dt.Rows(0).Item("recarga").ToString)
            ...
            lbl_compras.Text = ParseToDecimal.parse(dt.Rows(0).Item("compreas_desde").ToString)
            If lbl_compras.Text = String.Empty Then
                lbl_compras.Text = "0.00"
            End If
            dt = class_desde.gananciaS_desde(txt_fecha1.Text, txt_fecha2.Text)
            lbl_ganancias.Text = ParseToDecimal.parse(dt.Rows(0).Item("ganancias_desde").ToString)
            ...
            lbl_ventas.Text = ParseToDecimal.parse(dt.Rows(0).Item("vendas_desde").ToString)
            ...
            lbl_retirados.Text = ParseToDecimal.parse(dt.Rows(0).Item("total_retiros").ToString)
            ...
            lbl_total_de_ingresos.Text = ParseToDecimal.parse(total.ToString)
            ...
            lblGastosMenosIngresos.Text = ParseToDecimal.parse(ventasSinGasto.ToString)
            Dim totalCostoSinGastos As Decimal
            totalCostoSinGastos = Me.ganancias - Me.otrosGastos
            lblTotalMENOSGASTOS.Text = ParseToDecimal.parse(totalCostoSinGastos.ToString)
